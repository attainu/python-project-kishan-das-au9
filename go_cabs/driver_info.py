            ...
            if action == 1:
                ...
                if len(driver_info) == 1:
                    ...
                    while True:
                        ...
                        preference = int(
                            input(" 1: Update Location \n 2: Update Availabiltiy \n 3: Exit\n "))
                        if preference == 1:
                            DriverLocation()
                        elif preference == 2:
                            print("do you want to update your availability ? ")
                            isavailable = (
                                input("Enter y to update and n to not : "))
                            if isavailable == "y":

                                pref = int(
                                    input("Enter 1 if you are available and 0 if not : "))
                                driver_info = pickle.load(
                                    open("driver.dat", "rb"))
                                if pref == 1:
                                    DBHelper().updateisavailable(
                                        driver_info["driver_id"], 1)
                                else:
                                    DBHelper().updateisavailable(
                                        driver_info["driver_id"], 0)
                        elif preference == 3:
                            break
                        else:
                            print("Invalid input")
